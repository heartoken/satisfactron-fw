...
#define FIRMWARE_VERSION "25.07.27.1"
...
bool testServer(const char *testUrl = TEST_URL) {
  HTTPClient http;
  http.begin(TEST_URL);
  int code = http.GET();
  http.end();
...
void restoreNormalLEDs() {
  uint32_t color = (WiFi.status() == WL_CONNECTED && serverOK) ? strip.Color(0, 0, 255) : strip.Color(255, 0, 0);
...
void setup() {
  ...
  digitalWrite(LED_ERR_PIN, LOW);  // Start with error LED OFF
  ...
  while (!testServer("https://github.com") && retries < 20) {
    retries++;
    Serial.println("❌ BOOT: GitHub unreachable for ota retry #" + String(retries) + "/20");
    delay(500);
  }
  if (retries == 20) {
